...
int main(int argc, char **argv) {
	...
	if ((fdm = open(strPort, O_RDONLY | O_NONBLOCK)) == -1) {
	...
	while(1) {
		...
		if (ret!=-1) {
			if(evm.type == EV_REL) {
				if(evm.code == REL_X) {
					if(evm.value > 0) xp++;
					else if(evm.value < 0) xp--;
				}
				else if(evm.code == REL_Y) {
					if (evm.value > 0) yp++;
					else if(evm.value < 0) yp--;
				}
			...
			absolute_x+=xm;
			absolute_y-=ym;
			if (evm.type == EV_KEY && evm.value == 1) {
				pressed=1;
				pressedk=(rkp*10)+ckp;
				//printf("pressedk:%d\n", pressedk);
			}
			else if (evm.type == EV_KEY && evm.value == 0) {	
